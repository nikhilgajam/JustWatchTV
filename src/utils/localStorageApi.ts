...
const localStoreApi = {
  ...
  addPreviouslyWatchedData(video: any) {
    ...
    if (videoExists) {
      ...
      // Update the playingTime and index only if the values are than zero
      videoData.playingTime = video.playingTime > 0 ? video.playingTime : videoData.playingTime;
      videoData.index = video.index > 0 ? video.index : videoData.index;
